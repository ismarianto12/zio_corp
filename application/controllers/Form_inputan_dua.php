...
class Form_inputan_dua extends CI_Controller
{
	function __construct()
	{
		...
		$this->load->model(['Form_inputan_dua_model', 'm_user']);
	...
	public function json()
	{
		...
		if ($id_user != '' || $login != '') {
			header('Content-Type: application/json');
			echo $this->Form_inputan_dua_model->json();
		} else {
			show_404();
			die();
		}
	...
	public function tambah()
	{
		...
		$data = array(
			'page_title' => 'Tambah Form inputan dua',
			'button' => 'Create',
			'action' => site_url('form_inputan_dua/tambah_data'),
			'id' => set_value('id'),
			'nama' => set_value('nama'),
		...
		$this->template->load('template', 'form_inputan_dua/form_inputan_dua_form', $data);
	}
	...
	public function edit($id)
	{
		...
		if ($id_user != '' || $login != '') {
			...
			if ($row) {
				$data = array(
					'page_title' => 'Data FORM_INPUTAN_DUA',
					'button' => 'Update',
					'action' => site_url('form_inputan_dua/edit_data'),
				...
				$this->template->load('template', 'form_inputan_dua/form_inputan_dua_form', array_merge($data, $x));
			} else {
	...
	public function edit_data()
	{
		...
		if ($id_user != '' || $login != '') {
			...
			if ($this->form_validation->run() == FALSE) {
				...
				$this->Form_inputan_dua_model->update($this->input->post('id', TRUE), $data);
	...
	public function hapus($id)
	{
		...
		if ($row) {
			$this->Form_inputan_dua_model->delete($id);
	...
	public function excel()
	{
		...
		$namaFile = "form_inputan_dua.xls";
		$page_title = "form_inputan_dua";
		$tablehead = 0;
		$tablebody = 1;
		$nourut = 1;
		//penulisan header
	...
	public function word()
	{
		...
		header("Content-Disposition: attachment;Filename=form_inputan_dua.doc");

		$data = array(
			'form_inputan_dua_data' => $this->Form_inputan_dua_model->get_all(),
			'start' => 0
		);

		$this->load->view('template', 'form_inputan_dua/form_inputan_dua_doc', $data);
	}
}
